...
# MODELO NUEVO: CodeLlama - especializado en formatos estructurados
MODEL_ID  = os.getenv("MODEL_ID", "codellama/CodeLlama-7b-Instruct-hf")

# Inferencia
ATTEMPTS_PER_WIN = 3
N_WINDOWS_MAX    = 6
TEMP, TOP_P      = 0.3, 0.9
MAX_NEW          = 200  # Un poco más para JSON complejo

# Plausibilidad / tolerancias
H_MIN, H_MAX = 1.2, 2.2
W_MIN, W_MAX = 30, 300
...
def get_ov_model_and_tokenizer(model_id: str, device_pref: str = "CPU"):
    ...
    device = "CPU"
    print(f"Cargando CodeLlama en {device}...")
        ...
        try:
            ...
            out_ids = ov_model.generate(
                **inputs,
                ...
                pad_token_id=ov_model.config.pad_token_id,
            )
...
def run_chain_on_note(note_text: str, attempts_per_win=2, n_windows_max=4):
    cands=[]
...
for pid_name, system_text in PROMPTS.items():
    ...
    out_csv = f"{OUT_DIR}/pred_{pid_name}_{model_name}_n{len(notes)}.csv"
    rows=[]
    if pid_name == "v4_encadenado":
        for _, r in tqdm(notes.iterrows(), total=len(notes), desc=f"Inferencia {pid_name}"):
            ...
            out = run_chain_on_note(note, attempts_per_win=2, n_windows_max=4)
            rows.append({
